...
def find_bar(name):
    try:
        if name is None:
            raise ValueError("Bar cannot be null")
        
        bar=database.find_bar(name)
        if bar is None:
            return make_response("No such bar exists",404)
        return jsonify(bar)
...
@app.route('/api/beer-brewery', methods=['GET'])
def get_beer_brewery(beer):
    try:
        return jsonify(database.get_beer_brewery(beer))
    except Exception as e:
        return make_response(str(e), 500)
...
@app.route('/api/beerlikes', methods=['GET'])
def get_beer_likes():
    try:
        return jsonify(database.get_beer_likes())
    except Exception as e:
        return make_response(str(e), 500)
